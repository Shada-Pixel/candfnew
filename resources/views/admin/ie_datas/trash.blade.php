        ...
        <div class="flex-grow">
            <div class="">
                <ul class="flex gap-2 font-medium">
                    ...
                    <li class="">
                        <a href="{{route('btransactions.withdrawal')}}" class="hover:bg-white shadow-none rounded-t-lg px-6 py-2 flex items-center text-orange-400"><span><i class="mdi mdi-arrow-up mr-2 text-lg"></i>Withdrawal</span></a>
                    </li>
                    <li class="">
                        <a href="{{route('btransactions.trash')}}" class="card shadow-none rounded-b-none px-6 py-2 flex items-center text-red-400"><span><i class="mdi mdi-delete mr-2 text-lg"></i>Trash</span></a>
                    </li>
                </ul>
            </div>
            ...
            <div class="card rounded-tl-none">
                <div class="p-6">
                    ...
                    <h2 class="mb-4 text-xl">Trash Transaction</h2>
    ...
    <x-slot name="script">
        ...
        <script>
            ...
            var datatablelist = $('#transactionsTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{!! route('btransactions.trash') !!}",
                columns: [{
            ...
            function transactionRestore(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            method: 'PATCH',
                            url: BASE_URL + 'btransactions/' + id +'/restore',
            ...
            function transactionForceDelete(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            method: 'DELETE',
                            url: BASE_URL + 'btransactions/' + id +'/forcedelete',
                            success: function(response) {
                                $("#ajaxflash div p").text(response.success);
                                $("#ajaxflash").fadeIn().fadeOut(5000);
                                datatablelist.draw();
